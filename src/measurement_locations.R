...
wnv$popup <- apply(wnv,1,function(x){paste0(names(x),": ",x,collapse = "<BR>")})
...
sampleind <- sample(x = 1:nrow(wnv),replace = F,size = nrow(wnv)/100)
sampleset <- wnv[sampleind,]
m = leaflet(sampleset) %>% addTiles() %>% addMarkers(
  sampleset$Longitude, sampleset$Latitude, popup = sampleset$popup)
m
